...
#'
#' @param x numeric matrix ((C by r) by n): rows correspond to predictor variables for each class and columns correspond to the maximum number of samples over classes. Missing values are not allowed. 
#' @param y numeric matrix (C by n): rows correspond to response variables for each class and columns correspond to the maximum number of samples over classes. Missing values are not allowed. 
#' @param xtest a data frame or matrix (like x) containing predictors for the test set. 
#' @param ytest response for the test set. 
#' @param ntree numeric value: number of trees. 
#' @param sampsize numeric vector (C by 1): number of samples for each class of data. 
#' @param totsize Max number of samples across different classes 
#' @param mtry numeric value: number of predictors to be sampled at each node. 
...
#' @param cutoff (Classification only)  A vector of length equal to number of classes.  The winning class for an observation is the one with the maximum ratio of proportion of votes to cutoff. Default is 1/k where k is the number of classes  
...
#' @param localImp Should casewise importance measure be computed? 
#' @param nPerm Number of times the OOB data are permuted per tree for assessing variable importance.  Number larger than 1 gives slightly more stable estimate, but not very effectiv. Currently only implemented for regression. 
...
#' @param nclasses numeric value: the total number of classes C. 
...
#' 
#'   y<-matrix(0,2,max(n1,n2));  #--- create matrix (classes by n=max(n1,n2)) of response variable (expression of target gene j)
#'   y[1,seq(1,n1)]<-as.matrix(data1[j,])     
#'   y[2,seq(1,n2)]<-as.matrix(data2[j,])
#'   
#'   x<-matrix(0,p*2-2,max(n1,n2)) #--- create matrix of covariates (expression of genes)
#'   x[seq(1,p-1),seq(1,n1)]<-as.matrix(data1[-j,])
#'   x[seq(p,2*p-2),seq(1,n2)]<-as.matrix(data2[-j,])    
#'   
#'    jrf.out<-JRF(x=x,y=y,mtry=round(sqrt(p-1)),importance=TRUE,sampsize=nsample,nclasses=nclasses,ntree=ntree)
#'      
#'    imp1[-j,j]<-importance(jrf.out,scale=FALSE)[seq(1,p-1)]      #- save importance score for net1
#'    imp2[-j,j]<-importance(jrf.out,scale=FALSE)[seq(p,(p-1)*2)]  #- save importance score for net2
#'    
#' }
#' 
#' @useDynLib JRF, regRF, .registration = TRUE
#' @return out object of class JRF
#'
#' @export 
